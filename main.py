...
class Breakthrough:
    ...
    def __Save(self):
        try:
            s = open(str(input('File Location: ')), 'w')
            s.writelines(str(self.__Score))
            s.write('\n')
            s.writelines(str(self.__CurrentLock.Return_Challenges()))
            s.write('\n')
            s.writelines(str(self.__CurrentLock.Return_Challenges_Complete()))
            s.write('\n')
            s.writelines(str(self.__Hand.ReturnCards()))
            s.write('\n')
            s.writelines(str(self.__Sequence.ReturnCards()))
            s.write('\n')
            s.writelines(str(self.__Discard.ReturnCards()))
            s.write('\n')
            s.writelines(str(self.__Deck.ReturnCards()))
...
class Lock:
    ...
    def Return_Challenges(self):
        data = ""
        for x in self._Challenges:
            data += str(x.GetCondition())
        data = data.replace("][", ";")
        data = data.replace("[", "")
        data = data.replace("]", "")
        data = data.replace("'", "")
    ...
    def Return_Challenges_Complete(self):
        data = ""
        for x in self._Challenges:
            if x.GetMet():
                data += "Y"
...
class CardCollection:
    ...
    def ReturnCards(self):
        data = ""
        for x in range(self.GetNumberOfCards()):
            data += str(self.GetCardDescriptionAt(x))
            data += " "
            data += str(self.GetCardNumberAt(x))
            data += ","
        return data
